sudoku = __import__("Sudoku game", globals(), locals(),
                    [
                        "canvas",
                        "pen",
                        "line_color",
...
pen = sudoku.pen
line_color = sudoku.line_color
text_color = sudoku.text_color
background_color = sudoku.background_color
active = sudoku.active
board = sudoku.board
g = sudoku.g
...
def key_handler(event):
    try:
        ...
        if is_valid(char, *active):
            ...
            canvas.create_text(col * s + s / 2, row * s + s / 2, text=char, font=('Arial', 20), fill=text_color,
                               tags=('number', 'last'))
            board[row][col] = char
...
def write_number(n, row, col):
    s = float(canvas['width']) / g
    if board[row][col] != 0:
        canvas.create_text(col * s + s / 2, row * s + s / 2, text=str(n), font=('Arial', 20), fill=text_color,
                           tags='number')
